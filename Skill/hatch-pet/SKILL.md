...
This project-local copy lives at `<project-root>/Skill/hatch-pet`. Resolve `SKILL_DIR` from the selected `SKILL.md` location and use that project-local directory for bundled scripts and references; do not redirect script execution to another copy under `CODEX_HOME`.
...
```bash
SKILL_DIR="<absolute-project-root>/Skill/hatch-pet"
