...
- Qt 输入框左侧的 `＋` 打开系统多选文件窗口，可选择图片或普通文件；仍可直接粘贴剪贴板图片。没有附件时，上方预览区完全隐藏；存在附件时才展开，最多保留 8 个按添加顺序排列的卡片。图片显示缩略图，普通文件显示类型和文件名，每张卡片右上角的 `×` 只移除对应附件，超出宽度时横向滚动。
- 每张图片独立后台保存。发送会等待当前批次全部完成，然后在同一 MiMo 用户消息中按顺序提交多个 `image_url`；单张编码上限 10 MB、整批编码上限 30 MB。原始临时 PNG 尺寸不受缩略图影响。
- 手动选择的文件保留原始绝对路径，由 Agent 按任务需要通过本地读取工具打开；移除、发送、取消和退出绝不删除用户原文件。只有剪贴板产生且由 HomeAgent 拥有的临时 PNG 会自动清理。
- 文件选择完成后不在一个 UI 事件里批量完整解码图片：队列每轮只加入一个附件，图片通过 `QImageReader.setScaledSize` 直接读取约 120×74 的缩略图，再让出事件循环刷新界面。剪贴板预览使用快速缩放，原图 PNG 编码仍在后台线程完成。
- 当前附件面板位于输入框旁的同层水平布局，使用固定宽度统一容器，不会改变 composer 高度；原图 PNG 由 `ClipboardImageSaveWorker` 在线程中编码。保存期间点击发送会登记为“保存完成后自动发送”，不会阻塞 GUI 或提交不存在的路径。
- `HomeAgent.chat(..., image_path=...)` 接受本轮临时图片，`_image_message_content` 构造 `image_url + text` 的 MiMo 兼容内容数组，编码上限 10 MB。

## 无边框聊天窗口缩放（2026-07-24）

- `HomeAgentWindow` 的四条边和四个角各有透明 `FramelessResizeHandle`，使用 Qt `startSystemResize` 执行原生拖动缩放；平台不支持原生缩放时退回手动几何计算。
- 命中边缘时显示水平、垂直或对角缩放光标，最小尺寸仍为 640×300；标题栏原有拖动移动保持不变。
