...
async def start_assistant(_: web.Request) -> web.Response:
    ...
    assistant_process = subprocess.Popen([sys.executable, str(ROOT / "main.py"), "--config", str(CONFIG)], cwd=ROOT, stdout=assistant_log, stderr=subprocess.STDOUT, creationflags=flags)
...
async def test_tts(request: web.Request) -> web.Response:
    from src.ai_live_assistant.tts import TTSClient
...
async def test_llm(request: web.Request) -> web.Response:
    ...
    from src.ai_live_assistant.llm import LLMClient
