...
## 环境
- 共享父级 venv：`E:\Doc\AI直播\.venv`（Python 3.12 + torch **cu128**；5070 Ti 是 Blackwell sm_120，cu124 不兼容）
...
```bat
:: 1. 依赖(在父 .venv 中)
E:\Doc\AI直播\.venv\Scripts\python.exe -m pip install funasr modelscope sounddevice librosa soundfile mcp

:: 2. 下载模型权重(若未下载)
E:\Doc\AI直播\.venv\Scripts\python.exe download_model.py
```
...
"sound-asr": {
  ...
  "args": ["E:\\Doc\\AI直播\\Sound\\mcp_server.py"],
  "env": { "SENSEVOICE_MODEL": "E:\\Doc\\AI直播\\Sound\\models\\SenseVoiceSmall" }
}
```
...
## 已知坑（已修复，记录备查）
1. **中文路径**：模型放在 `E:\Doc\AI直播\...`（含中文），sentencepiece 的 C++ 后端读不到 bpe 词表。
