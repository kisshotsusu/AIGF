...
文字 / 剪贴板截图 / Sound MCP
  ...
  -> Qt 实时进度 / 长任务 TTS 汇报
  -> 家庭历史 / 高价值记忆 / Task 定时任务 / 自升级 task-recovery
```
...
普通聊天、桌面操作、文件任务和定时请求不创建 task-recovery。`begin_task` 只启动代码变更基线跟踪；规划模型确认 `domain=code` 且 `code_scope=self` 后才创建自升级恢复状态。直接重启仍绕过恢复持久化。启动时读到旧版本遗留的普通任务或“重启自己”任务会直接删除，不生成恢复提示。
