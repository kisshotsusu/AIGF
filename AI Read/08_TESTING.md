...
| `tests/test_self_programming_and_delivery.py` | 56 | 本地代码工具、按行读取、Codex stdin、规划路由、实现修改禁止误读屏、家庭系统提示隔离原始直播消息、直播消息清理短语、本地角色图库绝对路径输出、三视图文件名/ID/标签/主形象解析、图片分析路径传递、窗口/屏幕活动摘要脱敏、网易云通用播放/具体搜索区分、模型 UI 原子工具集、按计划裁剪媒体工具、停止/强制终止区分、绝对路径权限、伪工具调用、失败轮预算、重启恢复清理、自升级验证、原子写入与测试门禁 |
