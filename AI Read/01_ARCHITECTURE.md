...
语义规划器是普通消息的唯一任务意图与执行路线来源。本地代码不再用网页、站点、屏幕等关键词覆盖模型结论，只校验允许的字段、权限、安全边界和执行证据。直接重启等进程级控制指令仍在规划器之前本地处理，避免把重启自身交给大模型。

剪贴板截图由 Qt/Tk 输入层保存为系统临时 PNG，规划上下文只记录存在附件；执行请求把图片编码为本轮 `image_url` 数据消息直接交给 MiMo。Base64 和临时路径不进入持久历史，任务完成、取消或窗口退出后删除临时文件。
...
- `computer_control.full_access` 开启时，代码列举、读取、搜索、写入和替换均可使用用户指定的绝对路径；外部写入仍进入变更跟踪和文件语法校验，且不能读取或覆盖密钥文件。
- 自编程把实现、测试和 `AI Read` 视为同一交付单元；代码/配置变化但说明未同步时，`CodeEditorModule` 阻止验证通过。
## 模型驱动屏幕任务（2026-07-22）
